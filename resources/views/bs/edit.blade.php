            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Изменить BS</h3>
                    </div>
                    <div class="card-body">
                        {!! Form::model($bs, ['method' => 'PATCH','route' => ['bs.update', $bs->id]]) !!}
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control {{ $errors->has('number') ? 'is-invalid' : ''}}" name="number" required>
                                        @foreach($employees as $employee)
                                            <option @if($employee->number == $bs->number) selected @endif value="{{ $employee->number }}">{{ $employee->fio }}</option>
                                        @endforeach
                                    </select>
